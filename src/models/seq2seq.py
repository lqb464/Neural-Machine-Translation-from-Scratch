...
class Seq2Seq(nn.Module):
    ...
    def forward(
        ...
        max_len=None,
        input_seq=None,     # backward-compatible alias
        target_seq=None,    # backward-compatible alias
        temperature=1.0,
        top_k=None,
        top_p=None
    ):
        ...
        """
        # Support old argument names to avoid breaking existing code
        if src_tokens is None:
            src_tokens = input_seq
        if tgt_tokens is None and target_seq is not None:
            tgt_tokens = target_seq

        if src_tokens is None:
            raise ValueError("src_tokens (or input_seq) must be provided")
